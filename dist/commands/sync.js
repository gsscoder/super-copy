...
import { assertNoFlattenCollisions, globPattern, hasGlobstar, matchGlobstar, } from '../glob.js';
...
function isGitHubContentsEntry(v) {
    ...
    if (!('name' in v) || typeof v.name !== 'string')
        return false;
    if (!('download_url' in v) || typeof v.download_url !== 'string')
        return false;
    return true;
}
function isGitHubTreeEntry(v) {
    if (typeof v !== 'object' || v === null)
        return false;
...
function isGitHubTreeResponse(v) {
    if (typeof v !== 'object' || v === null)
        return false;
    if (!('tree' in v) || !Array.isArray(v.tree))
        return false;
    return true;
}
...
function repoRelativePath(subPath, workTreePath) {
    const base = subPath ? subPath.replace(/^\//, '') : '';
    return base ? `${base}/${workTreePath}` : workTreePath;
}
async function fetchGitHubFilesRecursive(owner, repo, subPath, fileSpec) {
...
function walkRelativeFiles(dir, base, acc) {
...
function resolveFilesRecursive(workTree, fileSpec) {
